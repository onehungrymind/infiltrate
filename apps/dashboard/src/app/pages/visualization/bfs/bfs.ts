...
export class BfsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChild('codeDisplay', { static: false }) codeDisplay!: ElementRef<HTMLDivElement>;
  ...
  readonly codeLines: CodeLine[] = [
    ...
    },
    {
      num: 11,
      content:
        '    <span class="comment">// Dequeue first node (FIFO)</span>',
    },
    {
      num: 12,
      content:
  ...
  drawGraph(): void {
    ...
    for (const [node, pos] of Object.entries(this.positions)) {
      ...
      (circle as unknown as { style: { filter: string } }).style.filter = 'drop-shadow(0 0 10px rgba(0, 255, 135, 0.3))';
  ...
  updateVisualization(): void {
    ...
    nodes.forEach((nodeEl) => {
      ...
      if (circleElement) {
        const circleWithStyle = circleElement as unknown as { style: { filter: string } };
        if (nodeName === step.current) {
          ...
          circleWithStyle.style.filter = 'drop-shadow(0 0 20px rgba(0, 255, 135, 0.8))';
          if (textElement) {
          ...
          circleWithStyle.style.filter = 'drop-shadow(0 0 10px rgba(0, 255, 135, 0.3))';
          if (textElement) {
            textElement.setAttribute('fill', '#ffffff');
          }
      ...
      if (nodeName in step.ranks) {
        ...
        badgeCircle.setAttribute('cx', (this.positions[nodeName].x + 18).toString());
        badgeCircle.setAttribute('cy', (this.positions[nodeName].y - 18).toString());
        ...
        badgeText.setAttribute('x', (this.positions[nodeName].x + 18).toString());
        badgeText.setAttribute('y', (this.positions[nodeName].y - 15).toString());
    ...
    edges.forEach((edge) => {
      ...
      if (!edge.hasAttribute('stroke-dasharray') || edge.getAttribute('stroke-dasharray') === 'none') {
