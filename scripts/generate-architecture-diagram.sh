...
# Generate guides/architecture-diagram.md with Mermaid diagram
# Usage: ./scripts/generate-architecture-diagram.sh

set -euo pipefail

OUTPUT_FILE="guides/architecture-diagram.md"
...
# Detect which apps use which libraries based on tsconfig paths
detect_dependencies() {
    local app="$1"
    local deps=()
    
    # Check if app has tsconfig that references libs
    if [ -f "apps/$app/tsconfig.app.json" ] || [ -f "apps/$app/tsconfig.json" ]; then
        # core-data provides HTTP services
        if [ -d "libs/core-data" ]; then
            deps+=("core-data")
        fi
        # core-state provides NgRx
        if [ -d "libs/core-state" ]; then
            deps+=("core-state")
        fi
        # material provides UI components
        if [ -d "libs/material" ]; then
            deps+=("material")
        fi
        # common-models provides types
        if [ -d "libs/common-models" ]; then
            deps+=("common-models")
        fi
    fi
